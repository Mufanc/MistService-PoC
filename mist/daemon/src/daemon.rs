...
use rsbinder::{Interface, ProcessState, SIBinder, Status, StatusCode, hub};
...
const DUMP_FLAG_PRIORITY_HIDE: i32 = 1 << 24;
...
fn with_service<T>(func: impl FnOnce(&dyn IMistService) -> anyhow::Result<T>) -> anyhow::Result<T> {
    ...
    let service = match hub::get_interface::<dyn IMistService>(SERVICE_NAME) {
        Ok(service) => service,
